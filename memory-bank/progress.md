...
3. 🟡 **PR 3 — `/projects/[slug]` + `/thoughts` routes + responsive/a11y polish** — next. Coming-soon templates, `generateStaticParams`. Migrate Pixxellent rich content from `/work` into `data/projects.ts`. Delete `src/app/work/`. Verify all breakpoints in both themes. Lighthouse pass. Final accent audit.
